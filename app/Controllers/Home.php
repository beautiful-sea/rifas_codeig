...
class Home extends BaseController
{
    ...
    public function index()
    {
        ...
        if(isset($id_c) && !empty($id_c)){
            ...
            $qtd_raffles_home = $userModel->select('qtd_raffles_home')->findAll();
            $qtd_raffle_home = $qtd_raffles_home[0];
            //pd($qtd_raffle_home->qtd_raffles_home);
            /* Busca direta, com limite de 4 */
            $data['raffles'] = $raffleModel->select('id, slug, title, images, price, status, draw_date, percent_level, fake_percent_level, description')->where(['status' => 1])->asObject()->orderBy('id','DESC')->findAll($qtd_raffle_home->qtd_raffles_home);
        }
        ...
        */
        ...
        $raffles = $raffleModel->select('id, winners, title, slug, draw_date, description,favoritar,images')->where(['status' => 2])->orderBy('id', 'DESC')->findAll();
        $data['rifafavorita'] = $raffleModel->select('id, winners, title, slug, draw_date, description,favoritar,images')->where(['favoritar' => 1])->orderBy('id', 'DESC')->findAll();
        ...
        if ($userLogotipo) {
            ...
            //return view('frontend/partials/header', ['url_da_imagem' => $url_da_imagem]);
        }
        $data['url_da_imagem'] = $url_da_imagem;

        // var_dump($data['winners'][0]['images']);
        // exit();
    ...
    // Checando ordens expiradas para exlcuir
    public function checkExpires()
    {
        $orderModel = new OrderModel();
        $orders = $orderModel->where('status',0)->orderBy('id', 'DESC')->findAll();

        // foreach($orders as $order){
        //     if(time() > intval($order->expires_in)){
        //         $this->cancelaOrdem($order->id);
        //     }
        // }
    }
    ...
    public function raffle($slug){
        ...
        if($data['raffle']){
            ...
            echo view('frontend/partials/header');
        ...
        foreach($data['orders'] as $orderKey => $orderValue){
            ...
            if ($orderValue->status == 0){
                ...
                if( time() < intval($orderValue->expires_in) ){
                    ...
                    // Cancelando pedido se estiver expirado
                    // $this->cancelaOrdem($orderValue->id);
                    // header("Refresh:0");
                }
